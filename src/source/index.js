...
//menampilkan homepage
app.get('/mahasiswa/homepage', async (req, res) => {
  try {
    const infos = await Info.find();
    const diskusis = await Diskusi.find();
    const reversedPosts = infos.reverse(); // Membalik urutan posting
    res.render('mahasiswa/homepage', { infos:reversedPosts });
  } catch (err) {
    console.error('Kesalahan:', err);
    res.sendStatus(500);
  }
...
app.post("/mahasiswa-pertanyaan",async (req, res)=>{
 ...
 await Diskusi.insertMany([data])
 res.redirect('diskusi/read-diskusi');
})

//menampilkan pertanyaan
app.get('/diskusi/read-diskusi', async (req, res) => {
  try {
    const diskusis = await Diskusi.find();
    const reversedPosts = diskusis.reverse(); // Membalik urutan posting
    res.render('mahasiswa/diskusi/read-diskusi', { diskusis:reversedPosts });
  } catch (err) {
    console.error('Kesalahan:', err);
    res.sendStatus(500);
  }
...
// Mengupdate data
app.get('/homepage/edit_pertanyaan/:id', (req, res) => {
  const id = req.params.id;
  Diskusi.findById(id)
    .then(data => {
      res.render('mahasiswa/diskusi/read-diskusi', { data });
    })
    .catch(err => console.log(err));
});

// Simpan perubahan item (dalam kasus edit)
app.post('/homepage/edit_pertanyaan/:id', async (req, res) => {
  try {
    await Diskusi.findByIdAndUpdate(req.params.id, req.body);
...
//menghapus pertanyaan diskusi
app.get('/homepage/hapus_pertanyaan/:id', (req, res) => {
  const id = req.params.id;
  Diskusi.findByIdAndDelete(id)
      .then(() => {
          res.redirect('/diskusi/read-diskusi');
...
const storage_minfo = multer.diskStorage({
  destination: './mahasiswa/info/read-info/uploads/',
  filename: function(req, file, cb) {
    cb(null, file.fieldname + '-' + Date.now() + path.extname(file.originalname));
  }
});

const upload_minfo = multer({
  storage: storage_minfo,
  limits: { fileSize: 10000000 }, // Batasan ukuran file: 10MB
}).single('image');

// Middleware untuk static files
app.use(express.static('mahasiswa/info/read-info'));

// Route untuk mengirim data postingan ke MongoDB

app.post('/read-info', (req, res) => {
  upload_minfo(req, res, (err) => {
    if (err) {
      res.status(500).send('Error uploading file');
      return;
    }
...
//menampilkan info
app.get('/info/read-info', async (req, res) => {
  try {
    const infos = await Info.find();
    const reversedPosts = infos.reverse(); // Membalik urutan posting
    res.render('mahasiswa/info/read-info', { infos:reversedPosts });
  } catch (err) {
    console.error('Kesalahan:', err);
    res.sendStatus(500);
  }
...
// Mengupdate data info
app.get('/mahasiswa/edit_info/:id', (req, res) => {
  const id = req.params.id;
  Info.findById(id)
    .then(data => {
      res.render('mahasiswa/info/edit-info', { data });
    })
    .catch(err => console.log(err));
});
...
//mahasiswa info
 // Pengaturan Multer untuk menyimpan gambar
 const storage_meinfo = multer.diskStorage({
  destination: './mahasiswa/info/edit_info/uploads/',
  filename: function(req, file, cb) {
    cb(null, file.fieldname + '-' + Date.now() + path.extname(file.originalname));
  }
});

const upload_meinfo = multer({
  storage: storage_meinfo,
  limits: { fileSize: 10000000 }, // Batasan ukuran file: 10MB
}).single('image');

// Middleware untuk static files
app.use(express.static('mahasiswa/info/edit_info'));

// Rute untuk menyimpan data edit postingan ke MongoDB
app.post('/edit_info/:id', (req, res) => {
  const postId = req.params.id;

  upload_meinfo(req, res, (err) => {
    if (err) {
       console.log(err);
      res.status(500).send('Error uploading file');
      return;
...
app.get("/mahasiswa/chatbot",(req, res)=>{
  res.render("mahasiswa/chatbot")
})
...
app.post("/mahasiswa-pertanyaan",async (req, res)=>{
 ...
 await Diskusi.insertMany([data])
 res.redirect('mahasiswa/hmp-mhs/diskusi/read-diskusi');
})

//menampilkan pertanyaan
app.get('/hmp-mhs/diskusi/read-diskusi', async (req, res) => {
  try {
    const diskusis = await Diskusi.find();
    const reversedPosts = diskusis.reverse(); // Membalik urutan posting
    res.render('admin/hmp-mhs/diskusi/read-diskusi', { diskusis:reversedPosts });
  } catch (err) {
    console.error('Kesalahan:', err);
    res.sendStatus(500);
  }
...
app.get('/homepage/hapus_pertanyaan/:id', (req, res) => {
  ...
  Diskusi.findByIdAndDelete(id)
      .then(() => {
          res.redirect('admin/homepage/diskusi/read-diskusi');
      })
      .catch(err => {
          console.log(err);
          res.status(500).send('Server Error');
...
//chatbot
app.post("/chatbot-send",async (req, res)=>{
...
app.get('/chatbot', async (req, res) => {
  try {
    const infos = await Info.find();
    const reversedPosts = infos.reverse(); // Membalik urutan posting
    res.render('admin/chatbot', { infos:reversedPosts });
  } catch (err) {
    console.error('Kesalahan:', err);
    res.sendStatus(500);
  }
